...
import { xarchon_version } from "../../../xarchon_version/xarchon_version";
...
export class KurtosisContext {
    ...
    private static async validateEngineApiVersion(genericKurtosisContext: GenericEngineClient): Promise<Result<null, Error>>{
        ...
        const libraryEngineSemver: semver.SemVer | null = semver.parse(xarchon_version)
        if (libraryEngineSemver === null){
            log.warn(`We expected the library engine version to match format X.Y.Z, but instead got ${xarchon_version}; this means that we can't verify the API library and engine versions match so you may encounter runtime errors`)
        }
